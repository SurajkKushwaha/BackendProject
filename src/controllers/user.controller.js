...
try {
  ...
  if( incomingRefreshToken!==user?.refreshToken)throw new ApiError(401,"refresh token is expired or used")
     ...
     const {newrefreshToken,accessToken}=await generateAccessAndRefreshTokens(user.id)
    const options = {
      httpOnly:true,
      secure:true
    }
...
const changeCurrentPassword = asyncHandler(async(req,res)=>{
  ...
  user.password = newPassword  
   ...
   .status(200)
   .json(
    new ApiResponse(200,{},"Password Changed Succssfully")
   )
})
...
if(!coverImageLocalPath) throw new ApiError(400,"cover image files is missing")
const coverImage= await uploadOnCloudinary(coverImageLocalPath)
if(!coverImage.url)throw new ApiError(400,"Error while uploading on cover iamge files")
